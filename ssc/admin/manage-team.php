...
var updateme = function(rowid){
	...
	$.ajax({
	  url: "ajax-update.php?type=team" + "&rowid="+tid + "&koname="+koname ,
...
<div id="search">
		...
		<span>
			<label for="teamname"> English Name:</label>
			<input name="teamname" id="teamname" value='<?=$_REQUEST['teamname']?>'/>
		</span>
...
<?
	...
	$sql =  "select count(*) cnt from team_data t where t.g_type=$game_type and t.t_name like '%$teamname%'";
	$row = mysql_fetch_row(mysql_query($sql,$conn) );
	$rowCnt = $row[0];
	$pageCnt = ceil($rowCnt /10);
	$section = floor($page / 10);
	$page = $page * 10;
	$sql = "select *  from team_data t where t.g_type=$game_type and t.t_name like '%$teamname%' limit $page, 10";
	$page = $page / 10;
...
$idx++; ?>
	...
	<tr id="<?= "trx".$idx ?>" tid="<?= $row['t_id'] ?>">
		<td class="gtype"><?= $row['g_type']?></td>
		<td class="ename"><?= $row['t_name']?></td>
		<td class="koname"><input type=text value='<?= $row['t_name_ko']?>'/></td>
		<td><input type=button value='update' onclick="updateme('<?="trx".$idx?>')"/></td>
...
<?
	if ( $section >0){
		echo "<span style='width:50px'>
			<a href='?game_type={$_REQUEST['game_type']}&teamname={$_REQUEST['teamname']}&page=",($section-1)*10,"}'>&lt;&lt;</a></span>";
	}
	
	for($idx = $section*10;$idx < $section*10+10; $idx++){
		echo "<span style='width:50px'>
			<a href='?game_type={$_REQUEST['game_type']}&teamname={$_REQUEST['teamname']}&page=$idx'>$idx</a></span>";
	}
