...
- (void)AllCheck:(id)sender
{
    ...
    [favoriteVC.team_lbl setText:NSLocalizedString(@"All Team", @"All Team")];
    favoriteVC.team_code = @"%";
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    NSString *m_strCountry = favoriteVC.location_code;
    NSString *strurl = nil;
    strurl = [NSString stringWithFormat:@"http://%@?table=3&type=%d&location=%@", g_Manager.host, [g_Manager g_Scope], m_strCountry];    
    strurl = [strurl stringByAddingPercentEscapesUsingEncoding:NSASCIIStringEncoding];
    //NSLog(@"team_url:%@", strurl);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [favoriteVC.team_lbl setText:[product objectForKey:@"name"]];
    favoriteVC.team_code = [product objectForKey:@"code"];
    [[self navigationController] popToViewController:favoriteVC animated:YES];
}
